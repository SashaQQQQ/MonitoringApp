...
function OverallOrdersPage({
  setAddOrderFormStatus,
  handleOrderClick,
  fetchOrders,
  orders,
}) {
  ...
  function renderItemList(order) {
    ...
    return (
      ...
      >
        <div
          className={`orderStatus ${order.progress_percent <= 20 ? "notStarted" : order.progress_percent < 80 ? "inProgress" : "completed"}`}
        ></div>
  ...
  return (
    <div className="ordersList">
      <nav className="filterNaviagtion">
        ...
        <h4
          className={filterType === "upcoming" ? "clicked" : ""}
          onClick={() => {
        ...
        <h4
          className={filterType === "ended" ? "clicked" : ""}
          onClick={() => {
            setFilterType("ended");
          }}
        >
